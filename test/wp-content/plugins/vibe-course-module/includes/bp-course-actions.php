...
class BP_Course_Action{
    ...
    function check_user_purchased_full_course($course_id,$user_id,$group_id,$args){
        ...
        }elseif( $check_partial_free_course_purchased == 1 ){
            ...
            if(!isset($done_flag) || !$done_flag){
                if(get_post_type($this->status_units[($k)]) == 'quiz'){
                    ...
                    if(is_numeric($quiz_status)){
                        ...
                        echo apply_filters('wplms_start_quiz_button','<a class="unit_button '.$quiz_class.'" data-quiz="'.$this->status_units[($k)].'"> '.__('Start Quiz','vibe').'</a>',$this->status_units[($k)]);
                    }
                }else{
